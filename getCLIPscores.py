...
def run():
	...
	for d_file in directions_list:
		...
		with torch.no_grad():
			...
			text_embeddings_diff = get_clip_text_embeddings(s_temp,clip_model)
			cos_sim = [ cosine_similarity(image_embeddings_diff,text_embeddings_diff[i]) for i in range(len(text_embeddings_diff))] 
			print(cos_sim)
